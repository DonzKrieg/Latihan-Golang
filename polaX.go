...
func printPolaBilangan(baris, kolom, n int) {
	...
	if n >= baris {
		if n >= kolom {
			if baris == kolom || (baris+kolom) == (n+1) { // Pengecekan Pengulangan kolom dan baris dengan operasi perbandingan,
			...
			fmt.Println()
			printPolaBilangan(baris+1, kolom-n, n) // gunakan fungsi rekursif pada baris ini
		}
	}
}
